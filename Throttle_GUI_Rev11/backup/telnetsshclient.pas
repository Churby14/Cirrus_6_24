...
begin
  Result := '';
  while Sock.CanRead(250) or (Sock.WaitingData > 0) do
  begin
    Sock.RecvPacket(250);
    TmpStr := Copy(SessionLog, FOutputPosition, Length(SessionLog));
    TmpInt := length(TmpStr);
    for i := 1 to length(TmpStr) do
        begin
        if (ord(TmpStr[i]) >= $20) and (ord(TmpStr[i]) <= $7F) then
